...
def generate_dashboard(results, history):
    ...
    for r in results:
        ...
        if url not in articles_map:
            articles_map[url] = {"title": r.get("article_title",""), "url": url, "images": [], "has_detection": False}
        articles_map[url]["images"].append(r)
        if r.get("logo_detected"):
            articles_map[url]["has_detection"] = True

    rl = {"high": "#F44336", "medium": "#FF9800", "low": "#4CAF50"}
    rn = {"high": "HIGH", "medium": "MEDIUM", "low": "LOW"}
    dc = ""
    for r in detected:
        ...
        dc += '<p class="cap-desc">' + r.get("cap_description","") + '</p>'
        dc += '<p class="recommendation">' + r.get("recommendation","") + '</p>'
        dc += '<span class="timestamp">' + r.get("timestamp","")[:16] + '</span>'
        dc += '</div></div>'
    ...
    for r in reversed(results[-100:]):
        ...
        tr += '<td class="status-cell">' + st + '</td>'
        tr += '<td><a href="' + r.get("article_url","#") + '" target="_blank">' + r.get("article_title","")[:40] + '</a></td>'
        tr += '<td>' + b + '</td>'
        tr += '<td>' + r.get("description","")[:50] + '</td>'
        tr += '<td><a href="' + r.get("image_url","#") + '" target="_blank">보기</a></td>'
        tr += '</tr>'

    empty_det = '<div class="empty-state"><p>감지된 로고가 없습니다</p></div>'
    empty_tbl = '<tr><td colspan="6" style="text-align:center;padding:40px;color:#444">분석 결과가 없습니다</td></tr>'
    det_section = "<div class='detected-grid'>" + dc + "</div>" if dc else empty_det
    tbl_section = tr if tr else empty_tbl
...
body{background:#0a0a0f;color:#e8e6e3;font-family:'Noto Sans KR',-apple-system,sans-serif;min-height:100vh}
.header{padding:32px 40px;border-bottom:1px solid rgba(255,255,255,0.06);background:linear-gradient(135deg,rgba(10,10,15,1),rgba(20,18,25,1));position:relative;overflow:hidden}
.header::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 60% 50% at 0% 0%,rgba(201,169,110,0.06),transparent 60%),radial-gradient(ellipse 40% 60% at 100% 100%,rgba(212,64,64,0.03),transparent 60%);pointer-events:none}
...
.logo-mark{width:48px;height:48px;border:2px solid rgba(201,169,110,0.6);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:900;color:#c9a96e;background:linear-gradient(135deg,rgba(201,169,110,0.1),transparent)}
.header h1{font-size:22px;font-weight:700;letter-spacing:-0.5px;color:#f0ede8}
.header .subtitle{font-size:12px;color:rgba(255,255,255,0.35);margin-top:4px;letter-spacing:0.3px}
.header-right{display:flex;gap:10px;align-items:center;flex-wrap:wrap}
.status-pill{display:inline-flex;align-items:center;gap:7px;font-size:11px;color:rgba(255,255,255,0.45);background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);padding:8px 16px;border-radius:8px}
.status-pill .live-dot{width:6px;height:6px;border-radius:50%;background:#4CAF50;animation:pd 2s infinite}
@keyframes pd{0%,100%{opacity:1}50%{opacity:0.3}}
.update-pill{font-size:11px;color:rgba(255,255,255,0.35);background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);padding:8px 16px;border-radius:8px}
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;padding:20px 32px}
.stat-card{padding:20px;border-radius:12px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06)}
.stat-card.alert{background:rgba(244,67,54,0.06);border-color:rgba(244,67,54,0.2)}
.stat-number{font-size:32px;font-weight:800}
.stat-number.red{color:#F44336}.stat-number.green{color:#66BB6A}.stat-number.gray{color:#888}
.stat-label{font-size:13px;color:#777;margin-top:4px}
.section{padding:20px 32px}
.section-title{font-size:16px;font-weight:700;margin-bottom:16px}
.detected-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:16px}
...
.brand-tag{font-size:13px;font-weight:700;color:#F44336;margin-bottom:8px}
...
.cap-desc{font-size:12px;color:#ef9a9a;line-height:1.5;margin-bottom:6px}
.recommendation{font-size:12px;color:#ffcc80;line-height:1.5;padding:8px 12px;border-radius:6px;background:rgba(255,152,0,0.08);border:1px solid rgba(255,152,0,0.15);margin-bottom:8px}
...
th{padding:12px 14px;text-align:left;background:rgba(255,255,255,0.03);color:#888;font-weight:600;font-size:11px;letter-spacing:0.5px;text-transform:uppercase;border-bottom:1px solid rgba(255,255,255,0.06)}
td{padding:10px 14px;border-bottom:1px solid rgba(255,255,255,0.03);color:#bbb}
...
.footer{padding:24px 40px;border-top:1px solid rgba(255,255,255,0.04);font-size:11px;color:#333;display:flex;justify-content:space-between}
@media(max-width:768px){.header{padding:20px}.stats{padding:16px;grid-template-columns:repeat(2,1fr)}.section{padding:16px}.detected-grid{grid-template-columns:1fr}.stat-number{font-size:24px}}
</style></head><body>
<div class="header"><div class="header-inner">
<div class="header-left"><div class="logo-mark">F</div>
<div><h1>Brand Safety Monitor</h1>
<div class="subtitle">F&amp;F Communications · 뉴스 이미지 브랜드 로고 자동 감지 시스템</div></div></div>
<div class="header-right">
<div class="status-pill"><span class="live-dot"></span> 실시간 모니터링 · 5분 주기</div>
<div class="update-pill">업데이트 """ + now + """</div>
</div></div></div>
<div class="stats">
...
<thead><tr><th>시각</th><th>상태</th><th>기사</th><th>감지 브랜드</th><th>설명</th><th>이미지</th></tr></thead>
<tbody>""" + tbl_section + """</tbody></table></div></div>
<div class="footer">
...
<span>Powered by Claude Vision API</span>
    ...
    with open("docs/index.html", "w", encoding="utf-8") as f:
        f.write(html)
    log.info("Dashboard generated: docs/index.html (detected %d / total %d)" % (len(detected), total))
